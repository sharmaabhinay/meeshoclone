    ...
    <>
    {
      products.length == 0 ? (<div className='w-full sm:my-32'>
        ...
        <div className='flex flex-wrap justify-between sm:gap-0 md:gap-6'>
          {
              products.map((item,i)=> (
                <Link to={`/singleproduct/${item._id}`} key={i} className='border-collapse border-[1px] text-[#353543] d:rounded-lg md:h-[25rem] flex flex-col md:gap-2 sm:w-[50%] md:w-56 overflow-hidden pt-px p-2' onClick={()=> setId(item._id)}>
                    <img src={item.image1} alt="" className='md:h-60 sm:h-40'/>
